...
CONFIG_FILE="$HOME/.retroarch/config/DinamicRetroarchConsola.cfg"
...
sed -i 's/\(savestate_auto_load *= *\).*/\1"false"/' $CONFIG_FILE
sed -i 's/\(savestate_auto_save *= *\).*/\1"false"/' $CONFIG_FILE
sed -i "s|\(savestate_directory *= *\).*|\1\""$savesDir"\"|" $CONFIG_FILE
sed -i 's/\(video_fullscreen_x *= *\).*/\1"320"/' $CONFIG_FILE
sed -i 's/\(video_fullscreen_y  *= *\).*/\1"240"/' $CONFIG_FILE
sed -i 's/\(video_smooth *= *\).*/\1"false"/' $CONFIG_FILE
sed -i 's/\(video_scale_integer *= *\).*/\1"true"/' $CONFIG_FILE
...
xrandr --output $output --mode "320x240x60.00"
...
sed -i 's/\(video_fullscreen_x *= *\).*/\1""/' $CONFIG_FILE
sed -i 's/\(video_fullscreen_y  *= *\).*/\1""/' $CONFIG_FILE
